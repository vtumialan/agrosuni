...
const getResults = async (page = 1) => {
    const response = await fetch(`api/providers?page=${page}`);
...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route('providers.destroy', id));
    }
}
</script>
<template>
                        ...
                        <table className="table-fixed w-full">
                            <thead>
                                <tr className="bg-gray-100">
                                    ...
                                    <th className="px-4 py-2">Proveedor (RUC)</th>
                            ...
                            <tbody>
                                <tr v-for="provider in providerData.data" :key="provider.id">
                                    ...
                                    <td className="border px-4 py-2">{{ provider.name }} ({{ provider.document_id }})</td>
